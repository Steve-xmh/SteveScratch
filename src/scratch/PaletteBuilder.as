...
public class PaletteBuilder {
	...
	public function showBlocksForCategory(selectedCategory:int, scrollToOrigin:Boolean, shiftKey:Boolean = false):void {
		...
		if (selectedCategory == Specs.steveBlocksCategory) return showSteveBlocksPalette(selectedCategory, catColor);
		if (selectedCategory == Specs.kindForNoobsCategory) return showKindForNoobsCategory(selectedCategory, catColor);
		
		if (app.viewedObj() && app.viewedObj().isStage) {
			// The stage has different blocks for some categories:
			var stageSpecific:Array = ['Control', 'Looks', 'Motion', 'Pen', 'Sensing', 'SteveBlocks'];
	...
	private function showKindForNoobsCategory(selectedCategory:int, catColor:int):void 
	{
		addItem(makeLabel(Translator.map('温馨提示：')));
		addItem(makeLabel(Translator.map('这里的模块可以在原版模块中搭建')));
		addItem(makeLabel(Translator.map('如果你需要加快速度可以使用这些')));
		addItem(makeLabel(Translator.map('除非你不需要移植到原版 Scratch 上！')));
		addItem(makeLabel(Translator.map('当然可以当作一个考验吧~')));
		addBlocksForCategory(selectedCategory, catColor);
		return;
	}
	...
	private function showSteveBlocksPalette(selectedCategory:int ,catColor:int):void 
	{
		addItem(makeLabel(Translator.map('警告！')));
		addItem(makeLabel(Translator.map('该模块不属于原生 Scratch 模块')));
		addItem(makeLabel(Translator.map('请确保不需要移植到')));
		addItem(makeLabel(Translator.map('原生 Scratch 再使用！')));
		addBlocksForCategory(selectedCategory, catColor);
		return;
	}
	...
	private function showMyBlocksPalette(shiftKey:Boolean):void {
		...
		addItem(new Button(Translator.map('Make a value block (Testing)'), makeNewValueBlock));
		var definitions:Array = app.viewedObj().procedureDefinitions();
		if (definitions.length > 0) {
			nextY += 5;
			for each (var proc:Block in definitions) {
				var b:Block
				if (proc.op == Specs.PROCEDURE_RETDEF)
				{
					b = new Block(proc.spec, 'R', Specs.procedureColor, Specs.FUNCALL, proc.defaultArgValues);
				}else {
	...
	protected function createVar(name:String, varSettings:VariableSettings):* {
		...
		if (varSettings.isCloudData) name = '\u2601 ' + name;
		if (obj.hasName(name)) {
	...
	private function makeNewValueBlock():void {//创建返回值模块
		function addBlockHat(dialog:DialogBox):void {
			var spec:String = specEditor.spec().replace(/^\s+|\s+$/g, '');
			if (spec.length == 0) return;
			var newHat:Block = new Block(spec, 'p', Specs.procedureColor, Specs.PROCEDURE_RETDEF);
			newHat.parameterNames = specEditor.inputNames();
			newHat.defaultArgValues = specEditor.defaultArgValues();
			newHat.defaultArgValues.push(0);
			newHat.warpProcFlag = specEditor.warpFlag();
			newHat.setSpec(spec);
			newHat.x = 10 - app.scriptsPane.x + Math.random() * 100;
			newHat.y = 10 - app.scriptsPane.y + Math.random() * 100;
		...
		var specEditor:ProcedureSpecEditor = new ProcedureSpecEditor('', [], false,BlockShape.NumberShape);
		var d:DialogBox = new DialogBox(addBlockHat);
		d.addTitle('New Value Block');
		d.addWidget(specEditor);
		d.addAcceptCancelButtons('OK');
		d.showOnStage(app.stage, true);
		specEditor.setInitialFocus();
