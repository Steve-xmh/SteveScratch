...
public class ScratchSprite extends ScratchObj {
	...
	public function ScratchSprite(name:String = null) {
		if (Worker.current.isPrimordial)
		{
			...
			objName = name || Translator.map('Sprite1');
		}
	...
	public function keepOnStage():void {//保证在舞台中
		
		if (false)
		{
			stageRect.width = ScratchObj.STAGEW;
			stageRect.height = ScratchObj.STAGEH;
			edgeBox.width = ScratchObj.STAGEW;
			edgeBox.height = ScratchObj.STAGEH;
	...
	public function setSize(percent:Number):void {
		...
		var ignoreDetect:Boolean = true;
		
		var minScale:Number = ignoreDetect ? 0 : Math.min(1, Math.max(5 / origW, 5 / origH));//可以试试 int.MIN_VALUE
		var maxScale:Number = ignoreDetect ? int.MAX_VALUE : Math.min((1.5 * ScratchObj.STAGEW) / origW, (1.5 * ScratchObj.STAGEH) / origH);
...
//			    }
	...
	public function menu(evt:MouseEvent):Menu {
		if (!Scratch.app.editMode && Scratch.isLoader){
			var fm:Menu = new Menu();
			fm.visible = false;
			return fm; 
		}
		var m:Menu = new Menu();
