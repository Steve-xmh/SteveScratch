...
public class ScratchRuntime {
	...
	private var projectClipboard:Clipboard;
	
	private var microphone:Microphone;
	private var timerBase:uint;

	public var projectToInstall:ScratchStage;
	...
	public function ScratchRuntime(app:Scratch, interp:Interpreter) {
		...
		{
			projectClipboard = Clipboard.generalClipboard
		}
		this.app = app;
		this.interp = interp;
		...
		//NativeDragManager.doDrag(app.stage, projectClipboard, null, null, NDO);
		
/*		FLASH::isFlash{
			if(NativeDragManager.isSupported && NativeDragOptions != undefined){
				var NDO:NativeDragOptions = new NativeDragOptions();
				NDO.allowCopy = false;
				NDO.allowLink = false;
				NDO.allowMove = true;
				app.stage.addEventListener(NativeDragEvent.NATIVE_DRAG_ENTER, onDragProjectFunction);
				app.stage.addEventListener(NativeDragEvent.NATIVE_DRAG_DROP, dragProjectFunction);
			}
		
		}
*/	}
/*	FLASH::isFlash{
		private function onDragProjectFunction(e:*):void 
		{
		...
		private function dragProjectFunction(e:*):void 
		{
			if(NativeDragManager.isSupported){
			var data:Array = e.clipboard.getData(ClipboardFormats.FILE_LIST_FORMAT) as Array;
			var file:File = data[0];
			
			function loadProjectFromDrag():void{
				
				installProjectFromFile(file.name.replace("." + file.extension, ""), file.data);
			}
			
			//传输过来的是 File 类数组
			if (file.extension == "sb" || file.extension == "ss2" || file.extension == "sb2" || file.extension == "sbx")
			{
				file.load();
				DialogBox.confirm(Translator.map('Replace contents of the current project?'), app.stage, loadProjectFromDrag);
			}
			}
		}
	}
*/	// -----------------------------
	...
	public function stepRuntime():void {
		...
		//为了程序执行速度与原生sc一致，所以添加延迟
		//trace(app.stepCount);
		if (app.stepCount >= app.originalFrameRate)
		{
			processEdgeTriggeredHats();
			interp.stepThreads();
			app.stagePane.commitPenStrokes();
			app.stepCount = 1;
		} else if (!app.usingOrginalSpeed) {//如果使用60fps速度
			processEdgeTriggeredHats();
			interp.stepThreads();
			app.stagePane.commitPenStrokes();
			app.stepCount = 1;
...
//-------- recording video code ---------
	...
	public function startVideo(editor:RecordingSpecEditor):void {
		...
		videoFramerate = app.stage.frameRate//(!editor.fifteenFlag()) ? 15.0 : 30.0;
		if (fullEditor) {
			videoFramerate=10.0;
		}
	...
	public function waitAndStart():void {
		...
		clearRecording();
		recording = true;
		var seconds:int = 3600; //modify to change length of video
...
//----------
	...
	public function startKeyHats(ch:int):void {
		...
		function startMatchingKeyHats(stack:Block, target:ScratchObj):void {
			if (stack.op == 'whenKeyPressed') {
				//if(typeof(stack.args[0].argValue) != 'String') return;
				var k:* = stack.args[0].argValue;
				if (k == 'any' || k == keyName) {
	...
	public function selectProjectFile():void {
		...
		function fileLoadHandler(event:Event):void {
			...
			else DialogBox.confirm(Translator.map('Replace contents of the current project?'), app.stage, doInstall);
		}
		function doInstall(ignore:* = null):void {
			if (app.bgWorker) 
			{
				ScWorkerClass.loadProjectUsingWorker(fileName, data);
				return;
			}
		...
		if (Scratch.app.isExtensionDevMode) {
			filter = [new FileFilter('ScratchX 工程', '*.sbx;*.sb;*.sb2')];
		}
		else {
			filter = [new FileFilter('Scratch 原生工程', '*.sb;*.sb2'), new FileFilter("SteveScratch 工程", "*.ss2"), new FileFilter("所有文件", "*.*")];
		}
	...
	public static function getKeyCode(keyName:String):int {
		trace(keyName);
		for each (var a:Array in BlockMenus.pressableKey){
			if (a[0] == keyName && a[1] != undefined) return a[1];
		};
