...
public class ScratchObj extends Sprite {
	...
	public static var STAGEW:int = 480;//480
	public static var STAGEH:int = 360;//360
	public static var CANVAS:Number = 1;
	...
	//字符串打印
	public var fontName:String = Resources.chooseFont(["Scratch"]);
	...
	SCRATCH::allow3d {
		if(this is ScratchStage || this is ScratchSprite || (parent && parent is ScratchStage)) {
			...
			if(reason == 0) {
				...
				id = (costume ? costume.baseLayerMD5 : null);
...
//						(this as ScratchStage).updateSpriteEffects((this as ScratchStage).videoImage, filterPack.getAllSettings());
	...
	public function procedureDefinitions():Array {
		...
		for (i = 0; i < scripts.length; i++) {
			b = scripts[i] as Block;
			if (b && (b.op == Specs.PROCEDURE_DEF)) result.push(b);
		}
		for (i = 0; i < scripts.length; i++) {
			b = scripts[i] as Block;
			if (b && (b.op == Specs.PROCEDURE_RETDEF)) result.push(b);
		}
		return result;
	}
	...
	public function lookupReturnProcedure(procName:String):Block {
		for (var i:int = 0; i < scripts.length; i++) {
			var b:Block = scripts[i] as Block;
			if (b && (b.op == Specs.PROCEDURE_RETDEF) && (b.spec == procName)) return b;
		}
		return null;
	}
