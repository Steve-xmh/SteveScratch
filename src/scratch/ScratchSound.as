...
public class ScratchSound {
	...
	public function ScratchSound(name:String = "undefined", sndData:ByteArray = null) {
		this.soundName = name;
	...
	private function reduceSizeIfNeeded(channels:int):void {
		...
		const maxRate:int = 192 * 2 * 1024;//最高比特率 // 192 * 1024
		if (rate > maxRate || channels == 2 || format == 'float') {
		// Convert stereo to mono, downsample if rate > maxRate, or both.
			var newRate:int = (rate > maxRate) ? maxRate : rate;//rate / 2
			var samples:Vector.<int> = WAVFile.extractSamples(soundData);
			if (rate > maxRate || channels == 2) {
