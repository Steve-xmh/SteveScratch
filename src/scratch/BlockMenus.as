...
public class BlockMenus implements DragClient {
	...
	public static const pressableKey:Array = [
		['space'],
		['up arrow'],
		['down arrow'],
		['right arrow'],
	...
	public static function BlockMenuHandler(evt:MouseEvent, block:Block, blockArg:BlockArg = null, menuName:String = null):void {
		if (!Scratch.app.editMode && Scratch.isLoader) return; 
		...
		if (menuName == null) { // menu gesture on a block (vs. an arg)
			...
			if ((op == Specs.PROCEDURE_DEF) || (op == Specs.CALL) || (op == Specs.PROCEDURE_RETDEF) || (op == Specs.FUNCALL)) menuName = 'procMenu';
		...
		if (menuName == 'penShape') menuHandler.penShapeMenu(evt);
		...
		if (menuName == 'spriteAndStage') menuHandler.spriteMenu(evt, false, false, true, true, false);
		...
		if (menuName == 'fontName') menuHandler.fontNameMenu(evt);
		if (menuName == 'hideOrShow') menuHandler.hideOrShowMenu(evt);
		if (menuName == 'valueOrList') menuHandler.valueOrListMenu(evt);
		
	}
	
	private function penShapeMenu(evt:flash.events.MouseEvent):void 
	{
		var m:Menu = new Menu(setBlockArg, 'hideOrShow');
		m.addItem(Translator.map("Circle"), "Circle");
		m.addItem(Translator.map("Square"), "Square");
		showMenu(m);
	}
	
	private function hideOrShowMenu(evt:flash.events.MouseEvent):void 
	{
		var m:Menu = new Menu(setBlockArg, 'hideOrShow');
		m.addItem(Translator.map("Hide"), "Hide");
		m.addItem(Translator.map("Show"), "Show");
		showMenu(m);
	}
	
	private function valueOrListMenu(evt:flash.events.MouseEvent):void 
	{
		var m:Menu = new Menu(setBlockArg, 'valueOrList');
		m.addItem(Translator.map("Variables"), "Variables");
		m.addItem(Translator.map("Lists"), "Lists");
		showMenu(m);
	}
	
	private function booleanMenu(evt:flash.events.MouseEvent):void 
	{
		var m:Menu = new Menu(setBlockArg, 'boolean');
		m.addItem(Translator.map("true"), "true");
		m.addItem(Translator.map("false"), "false");
		showMenu(m);
	}
	
	private function fontNameMenu(evt:flash.events.MouseEvent):void 
	{
		var m:Menu = new Menu(setBlockArg, 'fontName');
		m.addItem(Translator.map("Default"), "Default");
		m.addItem("Donegal");
		m.addItem("Gloria");
		m.addItem("Helvetica");
		m.addItem("Mystery");
...
//			handler.scrollAlignMenu(evt);
	...
	private function keyMenu(evt:MouseEvent):void {
		...
		//for (ch = 0; ch < 127; ch++) {
			//trace('msgid "' + String.fromCharCode(ch) + ' key"')
		//	m.addItem(String.fromCharCode(ch) + ' key', String.fromCharCode(ch)); // ascii keys
		//}
		for each (var s:Array in pressableKey) {
			var mi:MenuItem = m.addItem(Translator.map(s[0]) + (s[1] == undefined ? "" : " (" + s[1] + ")"), String.fromCharCode(s[1]));
		}
	...
	private function addGenericBlockItems(m:Menu):void {
		...
		if (!isInPalette(block)) {
			...
			m.addItem('copy to clipboard', copyJsonCode);
			m.addItem('make a block image', makeBlockImg);
	...
	private function makeBlockImg():void{
		const scaleX:Number = app.scriptsPane.scaleX;
		const scaleY:Number = app.scriptsPane.scaleY;
		var bm:BitmapData = new BitmapData((block.width + 20) * scaleX, (block.height + 20) * scaleY, true, 0);
		var fbm:BitmapData = new BitmapData((block.width + 20) * scaleX, (block.height + 20) * scaleY, false, 0xFFFFFF);
		bm.lock()
		fbm.lock()
		var dsf:DropShadowFilter = new DropShadowFilter(5,90,0,0.25,5 * scaleX,5 * scaleY,1,BitmapFilterQuality.HIGH);
		
		bm.draw(block, new Matrix(scaleX, 0, 0, scaleY, 10 * scaleX, 10 * scaleY), null, null, null, true);
		bm.applyFilter(bm,new Rectangle(0,0,bm.width,bm.height),new Point(),dsf);
		fbm.copyPixels(bm, new Rectangle(0, 0, bm.width, bm.height), new Point());
		Clipboard.generalClipboard.setData(ClipboardFormats.BITMAP_FORMAT, fbm);
	}
	...
	private function copyJsonCode():void 
	{
		if (!block) return;
		var cb:Clipboard = Clipboard.generalClipboard;
		cb.setData(ClipboardFormats.TEXT_FORMAT, "SSCB" + BlockIO.stackToString(block));
		return;
		var code:ByteArray = new ByteArray();
		code.writeUTFBytes(BlockIO.stackToString(block));
		code.compress();
		code.position = 0;
		var finalCode:ByteArray = new ByteArray();
		while (code.position < code.length){
			finalCode.writeUTFBytes(new Number(code.readByte() + 128).toString(16));
		}
		cb.setData(ClipboardFormats.TEXT_FORMAT, "SSCBE" + finalCode.toString());
	}
	...
	private function editProcSpec():void {
		var shape:int = BlockShape.CmdShape;
		if (block.op == Specs.CALL) {
			var def:Block = app.viewedObj().lookupProcedure(block.spec);
			if (!def){
				def = app.viewedObj().lookupReturnProcedure(block.spec);
				if (!def) return;
				shape = BlockShape.NumberShape;
			}
			block = def;
		}
		...
		if (shape == BlockShape.CmdShape)
		{
			...
			d.addTitle('Edit Value Block');
		}
		
		d.addWidget(new ProcedureSpecEditor(block.spec, block.parameterNames, block.warpProcFlag, shape));
