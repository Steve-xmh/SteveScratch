...
public class ScratchCostume {
	...
	public function ScratchCostume(
			name:String = "Unnameed", data:* = null, centerX:int = kCalculateCenter, centerY:int = kCalculateCenter, bmRes:int = 1) {
		costumeName = name;
	...
	public static function emptyBackdropSVG():ByteArray {
		var data:ByteArray = new ByteArray();
		data.writeUTFBytes(
				'<svg width="' + ScratchObj.STAGEW.toString() + '" height="' + ScratchObj.STAGEH.toString() + '"\n' +
				'  xmlns="http://www.w3.org/2000/svg" version="1.1"\n' +
				'  xmlns:xlink="http://www.w3.org/1999/xlink">\n' +
				'	<rect x="0" y="0" width="' + ScratchObj.STAGEW.toString() + '" height="' + ScratchObj.STAGEH.toString() + '" fill="#FFF" scratch-type="backdrop-fill"> </rect>\n' +
				'</svg>\n');
		return data;
	}
