...
public class ScratchStage extends ScratchObj {
	...
	public function ScratchStage() {
		objName = 'Stage';
		isStage = true;
		scrollRect = new Rectangle(0, 0, STAGEW * CANVAS, STAGEH * CANVAS); // clip drawing to my bounds
	...
	{
		scrollRect = new Rectangle(0, 0, STAGEW * CANVAS, STAGEH * CANVAS);
	}
	...
	public function addWhiteBG():void {
		bg = new Shape();
	...
	private function addPenLayer():void {
		...
		var bm:BitmapData = new BitmapData(STAGEW*CANVAS, STAGEH*CANVAS, true, 0);
		penLayer = new Bitmap(bm);
		//newPenStrokes.scaleX = newPenStrokes.scaleY = CANVAS;
		penLayer.scaleX = penLayer.scaleY = 1 / CANVAS;
		addChild(penLayer);
	}
	...
	public function scratchMouseX():int { return Math.max( -ScratchObj.STAGEW / 2, Math.min(mouseX - (STAGEW / 2), ScratchObj.STAGEW / 2)) }
	public function scratchMouseY():int { return -Math.max( -ScratchObj.STAGEH / 2, Math.min(mouseY - (STAGEH / 2), ScratchObj.STAGEH / 2)) }
	...
	public function drawString(str:String, posX:Number, posY:Number):void {
		commitPenStrokes();
		var bd:BitmapData = penLayer.bitmapData;
		var strP:Sprite = new Sprite();
...
//		}
		...
		function stamp2d():void {
			...
			m.scale(s.scaleX * CANVAS, s.scaleY * CANVAS);
			m.translate(s.x * CANVAS, s.y * CANVAS);
...
//	    		testBM.bitmapData = bmd;
	...
	public function commitPenStrokes():void {
		...
		penLayer.bitmapData.draw(newPenStrokes, new Matrix(CANVAS, 0, 0, CANVAS));
		newPenStrokes.graphics.clear();
		penActivity = false;
		SCRATCH::allow3d { if(Scratch.app.isIn3D) Scratch.app.render3D.updateRender(penLayer); }
	}
	...
	private function updateCachedBitmap():void {
		...
		if (!cachedBM) cachedBM = new BitmapData(STAGEW * CANVAS, STAGEH * CANVAS, false);
		cachedBM.fillRect(cachedBM.rect, 0xF0F080);
		cachedBM.draw(img);
		if (penLayer) cachedBM.draw(penLayer,new Matrix(CANVAS,0,0,CANVAS));
		if (videoImage) cachedBM.draw(videoImage,new Matrix(CANVAS,0,0,CANVAS));
...
//		}
	...
	public function updateInfo():void {
		info.stageW = ScratchObj.STAGEW;
		info.stageH = ScratchObj.STAGEH;
	...
	public function handleDrop(obj:*):Boolean {
		if ((obj is ScratchSprite) || (obj is Watcher) || (obj is ListWatcher)) {
			...
			if (obj is ScratchSprite) {
				...
				obj.setScratchXY(p.x - ScratchObj.STAGEW / 2, ScratchObj.STAGEH / 2 - p.y);
	...
	public override function readJSON(jsonObj:Object):void {
		...
		if (info.stageW)
		{
			ScratchObj.STAGEW = info.stageW;
		}else{
			ScratchObj.STAGEW = 480;
		}
		if (info.stageH)
		{
			ScratchObj.STAGEH = info.stageH;
		}else{
			ScratchObj.STAGEH = 360;
		}
		...
		for (i = 0; i < children.length; i++) {
			...
			if (!Worker.current.isPrimordial && Scratch.enabledWorker)
			{
				WorkerSideClass.wtm.send(["loadProcess", Translator.map("Loading Project..."), Translator.map("Reading Sprites Data")+" (" + i.toString() + "/" + children.length.toString() + ")", i / children.length]);
			}
		}
		...
		for each (var scratchObj:ScratchObj in allObjects()) {
			...
			if (!Worker.current.isPrimordial && Scratch.enabledWorker)
			{
				WorkerSideClass.wtm.send(["loadProcess", Translator.map("Loading Project..."), Translator.map("Instantiate Objects Data")+" (" + a.toString() + "/" + allObjects().length + ")", a / allObjects().length]);
			}
			a++;
		}
	}
