...
public class ProjectIO {
	...
	private static var translationStrings:Object = {
		imageLoadingErrorTitle: '素材加载失败',//Image Loading Error
		imageLoadingErrorHeader: '一些素材无法加载成功：',//At least one backdrop or costume failed to load:
		imageLoadingErrorBackdrop: '背景：{costumeName}',//Backdrop: {costumeName}
		imageLoadingErrorSprite: '角色：{spriteName}',//Sprite: {spriteName}
		imageLoadingErrorCostume: '造型：{costumeName}'//Costume: {costumeName}
	};
	...
	private function addImagesAndSounds(zip:ZipIO):void {
		...
		for (i = 0; i < images.length; i++) {
			...
			if (!Worker.current.isPrimordial && Scratch.enabledWorker) imgData.shareable = true;
			ext = ScratchCostume.fileExtension(imgData);
			zip.write(i + ext, imgData);
		}
		for (i = 0; i < sounds.length; i++) {
			var sndData:ByteArray = sounds[i][1];
			if (!Worker.current.isPrimordial && Scratch.enabledWorker) sndData.shareable = true;
	...
	protected function decodeFromZipFile(zipData:ByteArray):ScratchObj {
		...
		for each (var f:Array in files) {
			...
			{
				contents.shareable = true;
				WorkerSideClass.wtm.send(["loadProcess", Translator.map("Loading Project..."), "Reading JSON Data (" + index.toString() + "/" + files.length.toString() + ")", index / files.length]);
			}
	...
	private function installImagesAndSounds(objList:Array):void {
		...
		for each (var obj:ScratchObj in objList) {
			...
			for each (var c:ScratchCostume in obj.costumes) {
				...
				if (!Worker.current.isPrimordial && Scratch.enabledWorker)
				{
					WorkerSideClass.wtm.send(["loadProcess", Translator.map("Loading Project..."), "Loading Constumes ("
					+ oL.toString()
					+ "/"
					+ objList.length.toString()
					+ ")("
					+ rL.toString()
					+ "/"
					+ obj.costumes.length.toString()
					+ ")"
					, oL / objList.length * (rL / obj.costumes.length)]);
				}
				rL++;
			}
			rL = 0;
			for each (var snd:ScratchSound in obj.sounds) {
				...
				if (!Worker.current.isPrimordial && Scratch.enabledWorker)
				{
					WorkerSideClass.wtm.send(["loadProcess", Translator.map("Loading Project..."), "Loading Sounds ("
					+ oL.toString()
					+ "/"
					+ objList.length.toString()
					+ ")("
					+ rL.toString()
					+ "/"
					+ obj.sounds.length.toString()
					+ ")"
					, oL / objList.length * (rL / obj.sounds.length)]);
				}
				rL++;
			}
			oL++;
	...
	private function decodeImage(imageData:ByteArray, imageDict:Dictionary, doneFunction:Function, fail:Function):void {
		if (!Worker.current.isPrimordial && Scratch.enabledWorker) imageData.shareable = true;
	...
	private function decodeSVG(svgData:ByteArray, imageDict:Dictionary, doneFunction:Function):void {
		if (!Worker.current.isPrimordial && Scratch.enabledWorker) svgData.shareable = true;
	...
	public function downloadProjectAssets(projectData:ByteArray):void {
		if (!Worker.current.isPrimordial && Scratch.enabledWorker) projectData.shareable = true;
		function assetReceived(md5:String, data:ByteArray):void {
			...
			if (!Worker.current.isPrimordial && Scratch.enabledWorker) data.shareable = true;
			if (app.lp) {
