...
spriteFeaturesFilter.visible = false; // disable features filter for now
	...
	private function importImagesOrSpritesFromDisk():void {
		...
		try {
			...
			files.browse([new FileFilter("支持的资源文件(*.sprite2;*.jpeg;*.png;*.bmp;*.jpg;*.wav)","*.sprite2;*.jpeg;*.png;*.bmp;*.jpg;*.wav"), new FileFilter("所有文件(*.*)","*.*")]);
		} catch(e:*) {}
	}
	...
	private function importSoundsFromDisk():void {
		...
		try {
			// Ignore the exception that happens when you call browse() with the file browser open
			files.browse([new FileFilter("媒体文件(*.wav)", "*.wav"), new FileFilter("所有文件(*.*)", "*.*")]);//["媒体文件","mp3;wav"]
		} catch(e:*) {}
	}
	...
	protected function startSoundUpload(sndToUpload:ScratchSound, origName:String, whenDone:Function):void {
		if(!sndToUpload) {
			DialogBox.notify(
					'抱歉！',
					'这个音乐文件 '+origName+' 无法被 SteveScratch 解析，请使用 MP3 或 WAV 文件！',
					stage);
			return;
		}
		whenDone();
	...
	private function convertAndUploadSound(sndName:String, data:ByteArray):void {
		...
		if (snd && snd.sampleCount > 0) { // WAV data
			...
			if (app.lp) app.lp.setTitle('正在转换 MP3 文件...');
			var sound:Sound;
			function uploadConvertedSound(out:ScratchSound):void {
				snd = out;
				trace(snd, snd.sampleCount);
				if (snd && snd.sampleCount > 0) {
					startSoundUpload(out, origName, uploadComplete);
				}
				else {
					app.removeLoadProgressBox();
					DialogBox.notify('解码音乐错误', '抱歉，SteveScratch 无法读取 ' + sndName + '。', Scratch.app.stage);
				}
			}
			SCRATCH::allow3d {
				sound = new Sound();
