...
public class MotionAndPenPrims {
	...
	private function reWaitSec():void //延迟等待模块活动
	{
		//因为原版的帧率是30fps，在60fps上速度会加快，所以使用计时器来延迟指令
		if (!app.usingOrginalSpeed) return;//如果需要快速运行的话就不等待了
		if (interp.activeThread.firstTime) {
			interp.startTimer(1/app.stage.frameRate/30);
	...
	private function primMove(b:Block):void {
		...
		var d:Number = interp.numarg(b, 0);//(app.stage.frameRate/30);
