...
//	private static const blockLabelFormat:TextFormat = new TextFormat('LucidaBoldEmbedded', 10, 0xFFFFFF, true);
	...
	public var isReturnable:Boolean = false;
	...
	public var requestState:int = 0;		// 0 - no request made, 1 - awaiting response, 2 - data ready // 0 - 没有请求 1 - 等待请求 2 - 数据就绪
	...
	public function Block(spec:String = "undefined", type:String = " ", color:int = 0xD00000, op:* = 0, defaultArgs:Array = null) {
	...
	public function setSpec(newSpec:String, defaultArgs:Array = null):void {
		...
		if (op == Specs.PROCEDURE_DEF) {
		...
		} else if (op == Specs.PROCEDURE_RETDEF) {
			// procedure hat: make an icon from my spec and use that as the label
			indentTop = 20;
			indentBottom = 5;
			indentLeft = 5;
			...
			labelsAndArgs = [];
			argTypes = [];
			var label2:TextField = makeLabel(Translator.map('define value'));
			labelsAndArgs.push(label2);
			var b2:Block;
			labelsAndArgs.push(b2 = declarationBlock());
	...
	private function declarationBlock():Block {
		...
		for (var i:int = 0; i < parameterNames.length; i++) {
			...
			argType = (typeof(defaultArgValues[i]) == 'String' && defaultArgValues[i] == "f") ? 'f' : argType;
	...
	public function isEmbeddedInProcHat():Boolean {
		return (parent is Block) &&
			((Block(parent).op == Specs.PROCEDURE_DEF) || (Block(parent).op == Specs.PROCEDURE_RETDEF)) &&
	...
	private function argOrLabelFor(s:String, c:int):DisplayObject {
		// Possible token formats: 可能的变量标识符
		//	%<单个字符>
		//	%m.<菜单名字>
		//	@<图标名字>
		//	标签 (any string with no embedded white space that does not start with % or @)
		...
		if (s.length >= 2 && s.charAt(0) == "%") { // argument spec
			...
			if (argSpec == "b") return new BlockArg("b", c);//布尔
			if (argSpec == "c") return new BlockArg("c", c);//颜色
			if (argSpec == "d") return new BlockArg("d", c, true, s.slice(3));//未知
			if (argSpec == "m") return new BlockArg("m", c, true, s.slice(3));//特殊菜单
			if (argSpec == "n") return new BlockArg("n", c, true);//数字
			if (argSpec == "s") return new BlockArg("s", c, true);//字符串
	...
	private function makeLabel(label:String):TextField {
		...
		if (blockLabelFormat) text.defaultTextFormat = blockLabelFormat;
		text.text = label;
		if (useEmbeddedFont) {
	...
	public function toJsonCode(isTop:Boolean = false):* 
	{
		if (isProcDef() || op == 'proc_declaration') return "SSCB{}"; // don't duplicate procedure definition
		var forStage:Boolean = Scratch.app.viewedObj() && Scratch.app.viewedObj().isStage;
		var newStack:Block = BlockIO.stringToStack(BlockIO.stackToString(this), forStage);
		var result:String = "SSCB";
		var jsonObj:Array = [];
		...
		if (args.length > 0)
		{
			jsonObj[1] = new Array();
			var c:int = 0;
			for (var parg:String in args)
			{
				if (args[parg] is Block)
				{
					jsonObj[1][c] = args[parg].toJsonCode();
				}else if (args[parg] is BlockArg)
				{
					jsonObj[1][c] = args[parg].argValue;
				}
				c++;
			}
		}
		...
		if (nextBlock) jsonObj[2] = nextBlock.toJsonCode();//nextBlock
		if (subStack1) jsonObj[3] = subStack1.toJsonCode();//subStack1
		if (subStack2) jsonObj[4] = subStack2.toJsonCode();//subStack2
		
		if (isTop)
		{
			result += util.JSON.stringify(jsonObj, false);
			return result;
		}else{
			return jsonObj;
		}
