...
public class BlockShape extends Shape {
	...
	private var usedDP:Boolean = false;
	...
	private function lineStyle(g:Graphics):void{
		g.lineStyle(2, Color.mixRGB(color, 0, 0.1), 1, false, "normal", CapsStyle.SQUARE, JointStyle.MITER);
	}
	...
	private function drawBooleanShape(g:Graphics):void {
		var centerY:Number = topH / 2;
		//*
		//trace(topH, centerY, w);
		...
		/*g.drawPath(new Vector.<int>([1,2,2,2,2,2]),new Vector.<Number>( [
		centerY, topH,
		0, centerY,
		centerY, 0,
		w - centerY, 0,
	...
	private function drawNumberShape(g:Graphics):void {
		...
		g.moveTo(CornerInset, 0);
		g.lineTo(w - CornerInset, 0);
		g.lineTo(w, CornerInset);
		g.lineTo(w, topH - CornerInset);
		g.lineTo(w - CornerInset, topH);
		g.lineTo(CornerInset, topH);
		g.lineTo(0, topH - CornerInset);
	...
	private function drawProcHatShape(g:Graphics):void {
		...
		const topHeight:Number = Math.min(CornerInset * 2, w / 35);
		g.beginFill(Specs.procedureColor);
		g.moveTo(0, 15);
		g.lineTo(topHeight, 15 - topHeight);
		g.lineTo(w - topHeight, 15 - topHeight);
		g.lineTo(w, 15);
