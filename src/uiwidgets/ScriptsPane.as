...
return true; // xxx disable this check for now; it was causing confusion at Scratch@MIT conference
	...
	public function menu(evt:MouseEvent):Menu {
		...
		var str:String = cb.getData(ClipboardFormats.TEXT_FORMAT) as String;
		//trace(str);
		if (str.search("SSCB") !=-1 || str.search("SSCBE") !=-1)
		{
			m.addLine();
			m.addItem('paste blocks from clipboard', pasteBlocksFromClipboard);
		}
		return m;
	}
	
	public function pasteBlocksFromClipboard():void
	{
		var cb:Clipboard = Clipboard.generalClipboard;
		var str:String = cb.getData(ClipboardFormats.TEXT_FORMAT) as String;
		var startPos:int = str.search("SSCB");
		if (startPos !=-1)
		{
			str = str.slice(4);
			var b:Block = BlockIO.jsonToCmd(str);
			
			if (b) 
			{
				b.x = app.stage.mouseX;
				b.y = app.stage.mouseY;
				Scratch.app.gh.grabOnMouseUp(b);
			}
		}/*else if ((startPos = str.search("SSCBE")) != -1){
			str = str.slice(5);
			var ba:ByteArray = new ByteArray();
			ba.writeUTFBytes(str);
			ba.uncompress();
			var b:Block = BlockIO.jsonToCmd(ba.toString());
