...
public class DialogBox extends Sprite {
	...
	//public var bGG:Graphics = new Graphics();
	...
	public function DialogBox(acceptFunction:Function = null, cancelFunction:Function = null) {
		...
		//trace("New Dialog");
		backGround = new Bitmap(new BitmapData(Capabilities.screenResolutionX,Capabilities.screenResolutionY));
	...
	public function addAcceptCancelButtons(acceptLabel:String = null,cancelLabel:String='取消'):void {
	...
	public function addButton(label:String, action:Function):void {
		function doAction():void {
			var keep:* = false;
			if (action != null) keep = action();
			if (keep == false || keep == null) remove();
		}
	...
	private function updateBackground(e:Event = null):void
	{
		if (stage == null) return;
		const isVisible = visible;
		backGround.visible = false;
		visible = false;
		var bm:BitmapData = backGround.bitmapData;
		bm.lock();
		bm.draw(stage, null, new ColorTransform(.9, .9, .9) , null, null, false);
		bm.applyFilter(bm, new Rectangle(0, 0, bm.width, bm.height), new Point(), new BlurFilter(64,64,BitmapFilterQuality.HIGH));
		bm.unlock();
	...
	public function showOnStage(stage:Stage, center:Boolean = true, padding:Number = 0):void {
		...
		stage.addChildAt(this, stage.getChildIndex(this.backGround) + 1);
		stage.addEventListener(Event.RESIZE, updateBackground);
		updateBackground()
	...
	public function remove():void {
		if (backGround.parent != null) backGround.parent.removeChild(this.backGround);
	...
	private function drawBackground():void {
		...
		var titleBarColors:Array = [CSS.borderColor, CSS.borderColor];
		...
		bg.drawRect(0, 0, NativeWindow.systemMaxSize.x, NativeWindow.systemMaxSize.y);
		bg.endFill();
		*/
