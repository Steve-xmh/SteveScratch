...
public class MenuItem extends Sprite {
	...
	private var _label:TextField; // if _label is null, this item is a divider line
	public var checkmark:Shape;
	...
	public function setWidthHeight(w:int, itemHeight:int):void {
		...
		this.h = 1;
		if (_label) {
			this.h = Math.max(_label.height, itemHeight);
			_label.y = Math.max(0, (h - _label.height) / 2);
		}
		setBaseColor(menu.color);
	}

	public function isLine():Boolean { return !_label }

	public function getLabel():String { return _label ? _label.text : '' }
	...
	public function translate(menuName:String):void {
		if (_label && BlockMenus.shouldTranslateItemForMenu(_label.text, menuName)) {
			_label.text = Translator.map(_label.text);
		}
	}
	...
	private function addLabel(s:String, enabled:Boolean):void {
		...
		_label.defaultTextFormat = new TextFormat(CSS.font, CSS.menuFontSize, CSS.white);
		_label.text = s;
		_label.x = leftMargin;
		_label.y = 0;
		_label.alpha = enabled ? 1 : 0.5;
		w = _label.width + leftMargin + rightMargin;
		h = Math.max(_label.height, menu.itemHeight);
		addChild(_label);
		setBaseColor(menu.color);
	}
	...
	private function setHighlight(highlight:Boolean):void {
		...
		_label.textColor = highlight ? colorWithBrightness(menu.color, 0.3) : CSS.white;
