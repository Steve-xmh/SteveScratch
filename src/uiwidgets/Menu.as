...
public class Menu extends Sprite {
	...
	private var showingStage:Stage;
	...
	public function addItem(label:*, value:* = null, enabled:Boolean = true, checkmark:Boolean = false):MenuItem {
		...
		if ((!last || last.isLine()) && newItem.isLine()) return null;
	...
	public function selectedNativeMenu(itemValue:*):void {
		showingStage.focus = null;
...
/*		if (NativeMenu.isSupport && false)//如果支持原生菜单
		{
			var m:NativeMenu = new NativeMenu();
			
			function nMSelected(e:Event):void
			{
				//trace(e.target);
				var nm:NativeMenu = e.currentTarget as NativeMenu;
				var nmi:NativeMenuItem = e.target as NativeMenuItem;
				selectedNativeMenu(allItems[nm.getItemIndex(nmi)].selection);
			}
			
			for each (var mitem:MenuItem in allItems)
			{
				var mi:NativeMenuItem
				if (mitem.getLabel() != "")
				{
					mi = new NativeMenuItem(Translator.map(mitem.getLabel()));
					mi.name = mitem.getLabel();
					mi.checked = (mitem.checkmark != null ? mitem.checkmark.visible : false);
					m.addItem(mi);
					
				}else if (mitem.isLine()){
					mi = new NativeMenuItem("",true);
					m.addItem(mi);
				}
			}
			
			m.addEventListener(Event.SELECT, nMSelected);
			m.display(stage, stage.mouseX, stage.mouseY);
...
*/		
	...
	private function step(e:Event):void {
		...
		if ((localY < (2 + scrollInset)) && (firstItemIndex > 0)) scrollBy(-5);
		if ((localY > (this.height - scrollInset)) && (firstItemIndex < maxScrollIndex)) scrollBy(5);
	}
