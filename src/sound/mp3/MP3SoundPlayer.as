...
public class MP3SoundPlayer extends ScratchSoundPlayer {
	...
	private var playPosition:Number =-1;
	...
	public override function startPlaying(doneFunction:Function = null, rP:Number = 0):void {
		function loadDone(snd:Sound):void {
			mp3Sound = snd;
			startChannel(doneFunction, rP);
		}
		playPosition = rP;
		...
		else startChannel(doneFunction, rP);
	}

	private function startChannel(doneFunction:Function, startTime:Number = 0):void {
		var flashSnd:Sound = new Sound();
		flashSnd.addEventListener(SampleDataEvent.SAMPLE_DATA, writeSampleData);
		soundChannel = flashSnd.play(startTime * 1000);
		isLoading = false;
	...
	private function writeSampleData(evt:SampleDataEvent):void {
		...
		var n:int = mp3Sound.extract(buf, 4096, playPosition * 1000 * 44100);
		buf.position = 0;
		updateVolume();
