...
public class Specs {
	...
	public static const FUNCALL:String = "funCall";
	public static const FUNRETURN:String = "funReturn";
	public static const PROCEDURE_DEF:String = "procDef";
	public static const PROCEDURE_RETDEF:String = "retProcDef";
	...
	public static const steveBlocksCategory:int = 13;
	public static const kindForNoobsCategory:int = 14;
	...
	public static const categories:Array = [
	 // id   category name	color
		...
		[1,  "Motion",		0x4a6cd4 + 0x101010],
		[2,  "Looks",		0x8a55d7 + 0x101010],
		[3,  "Sound",		0xbb42c3 + 0x101010],
		[4,  "Pen",			0x0e9a6c + 0x101010], // Scratch 1.4: 0x009870
		[5,  "Events",		0xc88330 + 0x101010],
		[6,  "Control",		0xe1a91a + 0x101010],
		[7,  "Sensing",		0x2ca5e2 + 0x101010],
		[8,  "Operators",	0x5cb712 + 0x101010],
		[9,  "Data",		variableColor + 0x101010],
		[10, "More Blocks",	procedureColor + 0x101010],
		[11, "Parameter",	parameterColor + 0x101010],
		[12, "List",		listColor + 0x101010],
		[13, "SteveBlocks", 0x11CCFF],
		[14, "KindForNoobs", 0x22AAFF],
		//[15, "File System",  0x11AA11],
	...
	public static var commands:Array = [
		...
		["return %s",							"f", 10, FUNRETURN],
		
		// SteveBlocks
		["--"],
		["color on x:%n y:%n",								"r", 13, "getColorOn",							0,0],
		
		["--"],
		["play sound %m.sound on %n sec",					" ", 13, "playSoundOnPos",						"pop", 0],
		["play sound %m.sound on %n sec until done",		" ", 13, "doPlaySoundOnPosAndWait",				"pop", 0],
		["--"],
		//["set pen shape to %m.penShape",					" ", 13, "setPenShape",							"Circle"],
		["print %s on X:%n Y:%n",							" ", 13, "printText",							"我是文本~", 0, 0],
		["set print font to %m.fontName",					" ", 13, "setFont",								"Default"],
		["set print font color to %c",						" ", 13, "setFontColor",						0x000000],
		["set print font size to %n",						" ", 13, "setFontSize",							16],
		["set print font bold is %m.boolean",				" ", 13, "setFontBold",							false],
		["set print font italic is %m.boolean",				" ", 13, "setFontItalic",						false],
		["set print font underline is %m.boolean",			" ", 13, "setFontUnderline",					false], 
		
		["--"],
		["rotate axis x by %n",								" ", 13, "addRotX",								15],
		["rotate axis y by %n",								" ", 13, "addRotY",								15],
		["rotate axis z by %n",								" ", 13, "addRotZ",								15],
		["set axis x to %n",								" ", 13, "setRotX",								15],
		["set axis y to %n",								" ", 13, "setRotY",								15],
		["set axis z to %n",								" ", 13, "setRotZ",								15],
		["axis x",											"r", 13, "rotX"],
		["axis y",											"r", 13, "rotY"],
		["axis z",											"r", 13, "rotZ"],
		["--"],
		["set resolution to x:%n y:%n",						" ", 13, "setResolution",						480, 320],
		["--"],
		["set local data %s to %s",							" ", 13, "setLocalData",						"Key", "Data"],
		["get local data from %s",							"r", 13, "getLocalData",						"Key"],
		["--"],
		["%m.hideOrShow all %m.valueOrList in %m.spriteAndStage"," ", 13, "setAllValueListVisible",				"Hide","Variables","Stage"],
		["--"],
		//["Connect to SSCDS service ip: %s port: %n",		" ",13, "connectCloud",			"127.0.0.1",  8080], 
		// Kind for Noobs
		
		["%s == %s",										"b", 14, "==", "A", "a"],
		["if %b then %s else %s",							"r", 14, "ifThenElse",null , "true","false"],
		["--"],
		
		["upper %s",										"r", 14, "strToUpper",							"SteveScratch"],
		["lower %s",										"r", 14, "strToLower",							"sTEVEsCRATCH"],
		["invert %s",										"r", 14, "invertStr",							"SteveScratch"],
		["--"],
		
		["get unicode of %n to text",						"r", 14, "getUnicodeToText",					21992],
		["get text of %s to unicode",						"r", 14, "getTextToUnicode",					"嗨"],
		["get text from %n to %n in %s",					"r", 14, "getStrFromTo", 						6, 7, "Hello Scratch!"],
		["%n position of %s contain %s",					"r", 14, "posContainStr",						1, "我是文本~", "文本"],
		["--"],
		
		["将二进制的 %n 转换为十进制",					"r", 14, "binToDec",						1, 1010],
		["将二进制的 %n 转换为十六进制",					"r", 14, "binToHex",						1, 1010],
		["将二进制的 %n 转换为八进制",					"r", 14, "binToOct",						1, 1010],
		["将十进制的 %n 转换为二进制",					"r", 14, "decToBin",						1, 1010],
		["将十进制的 %n 转换为十六进制",					"r", 14, "decToHex",						1, 1010],
		["将十进制的 %n 转换为八进制",					"r", 14, "decToOct",						1, 1010],
		["将十六进制的 %n 转换为二进制",					"r", 14, "hexToBin",						1, 1010],
		["将十六进制的 %n 转换为十进制",					"r", 14, "hexToDec",						1, 1010],
		["将十六进制的 %n 转换为八进制",					"r", 14, "hexToOct",						1, 1010],
		
		["use %c to draw a square on x:%n y:%n width:%n height:%n",	" ", 13, "drawRect",					null,0,0,15,15],
		
		["--"],
		["hide all sprite %m.boolean",						" ", 14, "setAllSpriteVisible",					false],
		
		//File System
		//["request user to accept using File System"," ",15,"requestFSPomisition"],
		//["is accpeted to use File System","b",15,"accpetedUseFS"],
		//["--"],
		//["opening file list","r",15,"openingFile"],
	];
}}
