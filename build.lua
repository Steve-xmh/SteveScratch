...
local buildFormat = [["%s%s" -load-config+=%s -incremental=true +configname=%s%s --compiler.compress=%s --debug=%s -use-network=%s -optimize=%s --static-link-runtime-shared-libraries=true -accessible=true -o %s]]

local buildCmd = string.format(buildFormat,
compiler,compilerRun,configPath,TargetPlatform,title,"false",debugMod,"false",not debugmod,add)
...
buildCmd = string.format(buildFormat,
compiler,compilerRun,configPath,TargetPlatform,title,"true",debugMod,"false",not debugmod,add)
